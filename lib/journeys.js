...
const createJourney = (j, date) => {
	...
	// legs
	const preLegs = Array.isArray(j.train) ? j.train : [j.train]
	const legs = []
	let lastTime = 0
	...
	// price
	journey.price = (isNumber(+j.price) && +j.price > 0) ? {
		amount: +j.price,
		currency: 'EUR'
	} : null

	// info
	journey.info = j.obdobje || null
...
const journeys = (origin, destination, date = new Date()) => {
	...
	return soap.request('Iskalnik_mob', {
		...
		.then(res => {
			const rawJourneys = res.Iskalnik_mobResponse.Iskalnik_mobResult.results.connection
			if (Array.isArray(rawJourneys)) return rawJourneys
			if (rawJourneys) return [rawJourneys]
			return []
		})
		.then(res => res.map(x => createJourney(x, date)))
}
